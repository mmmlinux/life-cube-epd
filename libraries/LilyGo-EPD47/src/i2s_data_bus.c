/*
 * MODIFIED 2026-08-25 by Elliott Krimchansky for the life-cube-epd project.
 *
 * Changes from the upstream Xinyuan-LilyGO/LilyGo-EPD47 version of this file
 * are marked "LOCAL PATCH" inline; the repository README summarises them. They
 * build the direct-register I2S path (USER_I2S_REG) against ESP32 Arduino core
 * 3.x / IDF 5, which upstream does not.
 *
 * This file remains licensed under the GPLv3, as in the LICENSE file alongside.
 */
...
#include <soc/rtc.h>
// LOCAL PATCH: the USER_I2S_REG register path needs these; only the
// esp_lcd path was being built upstream, so they were never included.
...
#include <clk_ctrl_os.h>        // periph_rtc_apll_acquire
 ...
 *        the "done" interrupt.
 */
static gpio_num_t start_pulse_pin;   // LOCAL PATCH: used by the register path
...
void i2s_bus_init(i2s_bus_config *cfg)
{
    ...
    // LOCAL PATCH: IDF 5 replaced rtc_clk_apll_enable(enable, sdm0, sdm1, sdm2,
    // o_div) with a refcounted power acquire plus a coefficient setter, and the
    // APLL has to be powered before its coefficients can be written - doing it
    // the other way round hangs in epd_base_init().
    // 40 MHz xtal * (4 + sdm2) / (2 * (o_div + 2)) = 40 * 12 / 4 = 120 MHz.
    periph_rtc_apll_acquire();
    rtc_clk_apll_coeff_set(0, 0, 0, 8);   // o_div, sdm0, sdm1, sdm2
    //#else
...
static void gpio_setup_out(int32_t gpio, int32_t sig, bool invert)
{
    ...
    esp_rom_gpio_connect_out_signal(gpio, sig, invert, false);   // LOCAL PATCH: was gpio_matrix_out()
}
